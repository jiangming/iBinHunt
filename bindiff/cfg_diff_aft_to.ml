...
(*peng++try threaded read*)
let stp_queries_read = ref [];;
let testv_calls_read = ref [];;
let reg_ret_read = ref [];;
let quick_ret_read = ref [];;
let threaded_read_succ = ref [];;
let threaded_read (in_ch, a, b, c, d, flag) = 
  let (a,b,c,d) =
    (Marshal.from_channel in_ch : int * int * bool * bool)
  in 
  let () = Printf.printf "in threaded_read(), %d, %d, %b, %b\n" a b c d in
  let flag = true in 
    ()
;;
(*peng++try threaded read*)
	...
	then Printf.fprintf cfgdiff_debug_ch "reg_length1=%d, mem_length1=%d, reg_length2=%d, mem_length2=%d\n" reg_length mem_length (List.length post_reg2) (List.length post_mem2) in
      ...
      then
	    ...
	    (*let (stp_queries_read,testv_calls_read,reg_ret_read,quick_ret_read) = 
	      (Marshal.from_channel in_ch : int * int * bool * bool ) 
	    in*)
	    (*peng++ try thread read*)
	    let new_read_thread = 
	      Thread.create threaded_read (in_ch, stp_queries_read, testv_calls_read, reg_ret_read, quick_ret_read, threaded_read_succ)
	    in 
	    let () = Unix.sleep (1) in
	    (*let () = if (!threaded_read_succ = false)
	    then (
	      let () = Printf.printf "after 10 sec, threaded_read() fails--.\n" in
	      stp_queries_read := 0;
	      testv_calls_read := 0;
	      reg_ret_read := false; 
	      quick_ret_read := false;
	    )
	    in*)
	    let threaded_read_succ = false in
	    (*peng++*)(*--the following 4 variables been changed into ref--*)
	    let () = if cfg_debug_flag_level1
...
let cal_cfg_diff g1 g2 offset block_matchings test_type test_stp debug_str =
  ...
  let cfgdiff_debug_ch = open_out ("debug/cfgdiff." ^ debug_str) in
...
*)
	...
	try  ( 
		...
		if (fast_score > 0.9999999) then (
			let () = Printf.fprintf cfgdiff_debug_ch "Cfg_diff_score: fast matching blocks %s with %s, score=%f.\n" (Int64.to_string sa1) (Int64.to_string sa2) score_same_inst in
...
*)
      ...
      let () = Printf.fprintf cfgdiff_debug_ch "starting isomorphism\n\n" in
      let (iso_ret, iso_size, iso_score, iso_result) = if g_size1 > 90000000
        then Cfg_iso.isomorphism_accurate cfg1 cfg2 g_size1 (Hashtbl.create g_size1) 
          ...
          timeout_count_cg2 cfgdiff_debug_ch true
          ...
          timeout_count_cfg cfgdiff_debug_ch true
      in
