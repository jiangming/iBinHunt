...
(* sl is Vine.stmt list
  ...
  returns cmap : function name -> (function name, call num) list, where call num resets at 0 for each function
  	  fmap : function name -> Vine.Function
*)
			...
			List.fold_left 
				( fun (acc,num) s -> match s with
					| Vine.Call(_,Vine.Name(targ),_) -> (
						(((fix_name targ),num)::acc, num+1)
					)
					| _ -> 
			 		  	(acc,num+1)
				 ) ([],0) blksl 
		in
		Hashtbl.add h1 (fix_name v) calls;
		Hashtbl.add h2 (fix_name v) f
	  )
	 | Vine.Function(v,_,_,true,_) as f ->
		(  (*Printf.printf "Adding external function %s to cmap and fmap\n" v;*)
	      Hashtbl.add h1 (fix_name v) [];
	     Hashtbl.add h2 (fix_name v) f )
	 | _ -> ()
      ) sl in
  (h1, h2)
...
(*
  ...
  cg_int_tbl: vertexID -> vertex 
  returns	cg_ext_tbl: function name -> vertexID
*)
let create_callgraph (dl,sl) fun_tbl = 
 ...
 (*let () = Printf.printf "Starting to populate graph:\n" in*)
  ...
  let (g,name_2_vertex)  = List.fold_left 
	...
	with Not_found -> 
		( (*let () = Printf.printf "Warning: %s not found in cmap (in List.fold_left)\n" name in*)
		[] )
	) in 
	       List.fold_left 
