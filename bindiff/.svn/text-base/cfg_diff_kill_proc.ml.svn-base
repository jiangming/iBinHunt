...
let tid_pid_tbl = (Hashtbl.create 1024);; 
      ...
      then
        ...
        let reg_ret = if reg_length > 0
          then 
            ...
            in
						...
						Hashtbl.add tid_pid_tbl (Thread.id (Thread.self())) pid;
						let () = Printf.printf "pm: cfg_diff, after create_process,thread id=%d, process id=%d\n" (Thread.id (Thread.self())) pid in
						let () =  flush stdout in
	   ...
	   in
            ...
            let () = Unix.close err_fd2 in
						(*Unix.execv "kill" [|"-9 ";(string_of_int(pid))|];(*pm*)*)
        ...
        then
          ...
          let (mem_ret) = if mem_length > 0
            then 
              ...
              in
							Hashtbl.add tid_pid_tbl (Thread.id (Thread.self())) pid;
							let () = Printf.printf "pm2:pid =%d, before Unix.read in_fd1 buf\n"  pid  in
							let () =  flush stdout in
							
              let buf = String.create 6 in
              let buf_read = Unix.read in_fd1 buf 0 6 in
							let () = Printf.printf "pm2:pid =%d, after Unix.read in_fd1 buf\n"  pid  in
							let () =  flush stdout in
	   	 ...
	   	 in
	         let () = if cfg_debug_flag_level1 then Printf.printf "stp_queries=%d, testv_calls=%d, stp_true=%b, quick_true=%b, str=%s\n" stp_queries_read testv_calls_read mem_ret_read quick_ret_read debug_str in
            	...
            	let () = Unix.close err_fd2 in
							(*Unix.execv "kill" [|"-9 ";(string_of_int(pid))|];(*pm*)*)
							(*Unix.system ("kill -9 "^(string_of_int(pid)));(*pm*)*)
	       let () = cfg_stp_queries := !cfg_stp_queries + stp_queries_read in
	    let () = cfg_testv_calls := !cfg_testv_calls + testv_calls_read in
