...
(*
  (**  [hashtbl_of_dominators cfg dom] returns a hashtbl of the
       dominator relationship. Useful to cache dominators. *)
  let hashtbl_of_dominators cfg dom : ((S.elt,S.t) Hashtbl.t) = 
    let tbl = Hashtbl.create 65537 in 
      G.iter_vertex 
	(fun y -> let el = dom y in 
	   Hashtbl.add tbl y el) cfg;
      tbl

  (**  
       [idom_tbl_of_dom_tbl cfg tbl] returns an idom tbl such that
       Hashtbl.find idom x returns idom(x) if it exists, or raises
       Not_found if it has no such dominator.
       We make use of the following facts:
       - idom(n) <> n
       - idom(n) dominates n
       - idom(n) = d does not dominate any other dominator of n
       - the initial node r has no dominator
  *)
  let idom_tbl_of_dom_tbl  cfg ?(pred=G.pred) 
      (dom : (S.elt, S.t) Hashtbl.t) r = 
    let tmp = Hashtbl.create (Hashtbl.length dom) in 
    let domfind n = (try Hashtbl.find dom n with Not_found -> S.empty) in 
    let is_dominator_of d set =  (* true if d dominates at least one
                                    node of s, else false. 
                                    not(is_dominator_of x y)
                                    satisifies fact 3 *)
      let set = S.remove d set in 
	S.exists (fun node -> 
                    let node_dom = domfind node in 
                      S.mem d node_dom) set
    in
    let f n v = ( (* function used per node to calculate idom. *)
      if G.V.equal n r then 
	() (* r has no idom *) 
      else (
	match pred cfg n with
            [x] -> (* if a node has only one predecessor, it must be
                      the idom. Note: This isn't true if a node has no
                      dominators at all, e.g., is not reachable from
                      start. *)
              if S.is_empty (domfind n) then () else
		Hashtbl.add tmp n x
          | _ -> (
              let domin = domfind n in (* Fact 2: idom must dominate n *)
              let domin = S.remove n domin in (* Fact 1 *)
              let idom = S.filter (* Fact 3: if d dominates any other
                                     dominator of n, then it can't be
                                     the idom. *)
		(fun d -> 
                   (* fact 3 *)
                   let is_not_idom = is_dominator_of d domin in
                     not(is_not_idom)
		) domin
              in
              let idom = S.elements idom in
		match idom with
                    [x] -> Hashtbl.add tmp n x
                  | [] -> pwarn "No idom for node" 
                  | _ -> (* ivan: add exception for dominators? *)
                      failwith "Impossible: more than one idom"
            )
      ) 
    ) in
      Hashtbl.iter (f) dom;
