...
(* IVAN: Please look at this file. I don't think I fixed it
   correctly. *)
...
module D = Debug.Make(struct let name="Vine_memory2array" and default=`Debug end)
open D
...
let coerce_prog (dl,sl) = 
  ...
  let rec recurse_mem lst (dl,sl) = 
	  ...
	  ((_,name,Vine.TMem(r,_)) as mem)::ys -> 
	    ...
	    in 
	    (**  pwarn ("Deendizing memory for "^var_to_string mem
		     ^" |-> "^var_to_string arr);*) (**MJ*)
	      recurse_mem ys 
