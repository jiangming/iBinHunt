...
(*	method remove_bb b =
    self#check_bb b;
    self#check_graph;
		let succl = self#succ b in
		let predl = self#pred b in
		List.iter (fun pred->
			List.iter (fun succ->
				Printf.printf "pm: remove: %s->%s\n" (bbid_to_string (pred.id)) (bbid_to_string (succ.id)) ;
				self#add_edge pred succ
				)succl
			)predl;
		Printf.printf "pm:: remove_bb:%s predl.length=%d, succ.length=%d\n" (bbid_to_string(b.id)) (List.length predl) (List.length succl);
...
(*********************************pm*****************************************)
let print_ir_l sll = 
	let () = Printf.printf "print_ir_l sll\n" in
(*let out_vineIR = open_out "debug/vineIR_sll" in*)
let () = List.iter(fun (sl,a)->
(*sl start*)
	...
	List.iter (fun s3 ->
		(*match s3 with 
			...
			| Jmp(expr) ->
			  ...
			  output_string out_vineIR "\"-\n";
			| _ -> 
				output_string out_vineIR "\"no matched stmt: s=\n" ;
				output_string out_vineIR (stmt_to_string s3);
				output_char out_vineIR '\n';
				*)
				Printf.printf "print_ir_l: %s" (stmt_to_string s3);
	) sl;
	Printf.printf "print_ir_l: one blk over\n";
(*s end*)
)sll
in
()
...
(*pm print cfg*)
(*
let print_cfg_debug current_cfg =
    ...
    let () = List.iter (fun bblock_id ->      
		...
		let () = List.iter(fun stmt->
			Printf.printf  "pm_cfg: %s\n" (stmt_to_string stmt)
			)stmts in
		Printf.printf "pm_cfg: one blk end\n"
...
let stmts_to_bb (dl:decl list) (sl:stmt list) : stmt list cfg = 
  ...
  let rec add_edges bb next s' =
    match s' with
	    Jmp _ 
	  | CJmp _ -> 
			(*let () = Printf.printf "add_edges->Jmp: %s" (stmt_to_string s') in (*pm*)*)
  ...
  in
	(*(*pm debug*)
	let () = List.iter(fun bb->
		let () = List.iter(fun s->
			Printf.printf "revblocks:s=%s\n" (stmt_to_string(s))
		)(bb.stmts) in
		Printf.printf "one blk end\n"
		)revblocks in
	(*pm debug end*)*)
  let first =
