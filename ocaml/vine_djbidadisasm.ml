...
let call_nonfunction = ref 0;;
let call_function = ref 0;;
let call_indirect = ref 0;;
...
let read_function_file file = 
  ...
  let read_one_line () = 
    try
      Scanf.bscanf scanbuf "0x%x 0x%x %[^\n]\n" 
	(fun x y z -> 
	  let finfo = 
	    {start_address = (Int64.of_int x);
	     end_address = (Int64.of_int y);
	     name = z;
	     is_extern = false; } in
...
let irtbl_to_functions irtbl = 
	...
	match (finfo.is_extern,(List.length irstmts)) with
	    true, 0 -> (* an extern function with no irstmts, just
	  ...
	  let addr = label_to_addr l in 
		...
		Not_found ->  (* this could be a call to a block
		  ...
		  ( Printf.eprintf "Warning: call to non-function: %s\n"
		      (stmt_to_string s);
		    call_nonfunction := !call_nonfunction + 1;
...
let to_irfunctions instrs funcs =
  ...
  let fl = List.map (fun x -> 
    match x with 
	Function(_, _, _, false, _) -> flatten_function_scope x
  ...
  let _ = Vine_typecheck.typecheck_jmp_targets := false in 
  let _ = Vine_typecheck.typecheck (dl, fl) in
  let _ = Vine_typecheck.typecheck_jmp_targets := true in 
...
let elf_to_irfunctions filename =
  ...
  let instrs = List.sort (fun (a,_) (b,_) ->
    Int64.compare a b) instrs in 
...
(** @param stmts a stmt list
    @param addrs_to_fnnames PMap of addresses to function names,
    as created by read_imp_table.
    @return stmts with external function declarations added to head,
    and call-attr'd-indirect jmps rewritten to Calls. Also a list
    of the referenced external function names.
*)
let ijmps_to_call_externs stmts addrs_to_fnnames =
  let vis = 
object (self)
  inherit nop_vine_visitor
  val mutable it_addr = 0l
  val mutable used_fnnames = StringSet.empty
  method get_used_fnnames = used_fnnames
  method visit_stmt stmt =
    match stmt with
    | Comment(c) ->
        let f = (fun x -> it_addr <- x) in
        (try
           Scanf.sscanf 
             c
             "%_lx: call *%lx"
    ...
    | Jmp(e) when it_addr <> 0l ->
        let fnname = 
          if PMap.mem it_addr addrs_to_fnnames then
            PMap.find it_addr addrs_to_fnnames
          else
            Printf.sprintf "ext_%lx" it_addr
        in
        used_fnnames <- StringSet.add fnname used_fnnames;
        it_addr <- 0l;
        ChangeTo(Call(None, fnname, []))
    | Function _ ->
        (* add a Ret to end of each fn *)
        ChangeDoChildrenPost
          (stmt, 
           (fun stmt ->
              match stmt with
              | Function(v,t,dl,e,Some(Block(bdl,sl))) ->
                  Function(v,t,dl,e,Some(Block(bdl, sl @ [Return(None)])))
              | _ -> 
                  stmt))
    | _ ->
        DoChildren
  ...
  let stmts = 
    List.map 
      ...
      stmts
  in
  stmts, StringSet.elements vis#get_used_fnnames
