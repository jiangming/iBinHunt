...
class pp ?(debug_labels=false) ft =
  ...
  and newline = Format.pp_print_newline ft in
  let comma () = pp ","; space() in 
object(self)
inherit Vine_pp.pp ~with_varids:false ft as vine_pp
 ...
 (** Pretty print a program using the given formater *)
  method format_program (dl,sl) =
    ...
    newline();
    (* should be #defined to something like "int64_t vine_ir_program() {" *)
    pp "VINE_PROG_START";
    newline();
    open_box 2;
    open_hvbox 0;
    ...
    newline();
    close_box();
    (* Sholud be #defined to something like "}" *)
    pp "VINE_PROG_END"
