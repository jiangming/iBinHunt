...
let translate fileName fileType =
...
let flag_descope = ref true in   (*default was true*) 
let flag_save_globals = ref true in  (*default was true*)
...
let () = match fileType with
	| "Elf" -> trans_add (SetFile(Elf(fileName)))
	| "IdaDB"-> trans_add (SetFile(IdaDB(fileName,1)))
	| _ -> ()
in
...
(*
let () = opt_flags := "constprop" :: !opt_flags in

let () = opt_flags := "gvn" :: !opt_flags in
*)
