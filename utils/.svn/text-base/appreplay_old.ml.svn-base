...
let appreplay args trace =
	...
	let d_cfg = Vine_cfg.prog_to_cfg trace in
	let () = print_cfg d_cfg "dynamic_cfg_all" in
	
	let s = args.trace_file in
	let idx = String.index s '.' in
	let s = String.sub s 0 idx in 
	let sl_ch = open_out ("ir__"^s) in
	let () = Marshal.to_channel sl_ch sl [] in
	let () = close_out sl_ch in
