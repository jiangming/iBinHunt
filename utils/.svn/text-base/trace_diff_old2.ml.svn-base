...
let slight_diff s_cfg id block_match_l seq=
...
let find_unmatch s_cfg_cover_list1 s_cfg_cover_list2 s_cfg1 s_cfg2 =
	...
	let () = List.iter(fun id1 -> 
		...
		with Not_found -> 
			(
				let () = slight_diff s_cfg1 id1 block_match_l 1 in
	...
	let () = List.iter(fun id2 -> 
		...
		with Not_found -> 
			(
				let () = slight_diff s_cfg2 id2 block_match_l 2 in
...
(**cmp*)	
  ...
  ;
	*)
	...
	let () = find_unmatch s_cfg_cover_list1 s_cfg_cover_list2 cfg1 cfg2 in
	let () = Printf.printf "s_cfg_cover_list1 len=%d s_cfg_cover_list2 len=%d\n" (List.length s_cfg_cover_list1) (List.length s_cfg_cover_list2) in
	let () = Printf.printf "before_match_num=%d,after_match_num=%d\n" before_match_num after_match_num in
