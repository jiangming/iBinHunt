...
let print_bb_color bb_tag_tbl trace_id=
	...
	let () = Hashtbl.iter (fun id total_color -> 
		let () = Printf.fprintf app_debug_ch  "\nprint_bb_color: %s total_color=%d\n" (bbid_to_string(id)) total_color  in
		()
		) bb_tag_tbl 
	in
()
;;
(****************************************************************************************)
(*let rec calc_suc cfg id_list ref_suc_list=
	let () = List.iter(fun id -> 
		let suc_l = bb_succ cfg id in
		ref_suc_list := suc_l @ !ref_suc_list;
		let () = calc_suc cfg suc_l ref_suc_list in
		()
		)id_list 
	in
	()
...
let appreplay args trace ida_file (*trace2 ida_file2 taint_range_file*)=
  ...
  let d_cfg1 = Vine_cfg.prog_to_cfg trace in
	...
	let bb_suc_tbl = Hashtbl.create 100 in
	
	let ref_exist_bb_l = ref [] in
	(**iternal func*)
let internal_filter (id:Vine_cfg.bbid)=
	let ret = try(
		let _ = List.find (fun x -> (x=id)) (!ref_exist_bb_l) in
		false
		)
	with Not_found ->
		(
			ref_exist_bb_l := id::(!ref_exist_bb_l);
			true
			) 
	in
ret
...
let rec calc_suc id_list ref_num level =
	let _ = if (level < 80) then
		(
			let () = List.iter(fun id -> 
					let suc_l = bb_succ s_cfg1 id in
					let suc_l = List.filter internal_filter suc_l in
					ref_num := (List.length suc_l)+(!ref_num);
					let () = calc_suc  suc_l ref_num (level+ 1) in
					()
			)id_list 
			in
			()		
...
in
	...
	let () = List.iter (fun d_blk_id->
		...
		let () = List.iter(fun d_stmt->
			(*let () = Printf.fprintf app_debug_ch "d_stmt: %s \n"  (stmt_to_string(d_stmt)) in*)
			ref_suc_list := [] ;
			ref_num := 0;
		let () = match d_stmt with
			...
			|Move(Temp(n,"post",t),BinOp(BITAND, lval, e))->
				ref_exist_bb_l := [];
					let () = match !is_low with
						| true->
							(*let () =  calc_suc s_cfg1 [!s_blk_id] ref_suc_list in*)
							let () = try(
								 	let num = Hashtbl.find bb_suc_tbl !s_blk_id in
									let () = Printf.fprintf app_debug_ch "suc_num:%09d s_blk_id=%s  constrain: %s \n" num (bbid_to_string(!s_blk_id)) (stmt_to_string(d_stmt)) in
									let () = Printf.printf "suc_num:%09d s_blk_id=%s constrain: %s \n" num (bbid_to_string(!s_blk_id)) (stmt_to_string(d_stmt)) in
									()
								)
							with Not_found -> 
								(
									let () =  calc_suc [!s_blk_id] ref_num 0 in
									let () = Printf.fprintf app_debug_ch "suc_num:%09d s_blk_id=%s  constrain: %s \n" !ref_num (bbid_to_string(!s_blk_id)) (stmt_to_string(d_stmt)) in
									let () = Printf.printf "suc_num:%09d s_blk_id=%s constrain: %s \n" !ref_num (bbid_to_string(!s_blk_id)) (stmt_to_string(d_stmt)) in
									()	
								)
							in
