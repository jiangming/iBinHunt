...
void usage( char *progname )
{
    ...
    printf("\tx - Print only Vine IR in XML\n");
}
...
// void print_decls(const map<string, reg_t> &decls)
// {
//   for(map<string, reg_t>::const_iterator it = decls.begin();
//       it != decls.end(); it++){
//     cout << "\tvar " << it->first << ":" 
// 	 << Exp::string_type(it->second) << ";" << endl;
//   }
// }



void print_xml(asm_program_t *prog, vector<vine_block_t *> vblocks )
{
  IR2XML xmler(cout);
  xmler.xml_prologue();
  for (vector<vine_block_t*>::iterator i = vblocks.begin();
       i != vblocks.end(); i++ ) {

      vector<Stmt *> *inner = (*i)->vine_ir;
      
      cout << "<!-- ";
      ostream_insn(prog, (*i)->inst, cout);
      cout << " -->" <<endl;
      
      for (vector<Stmt*>::iterator j = inner->begin(); j != inner->end(); j++ )
	(*j)->accept(&xmler);
  }
  xmler.xml_epilogue();
}
...
void print_deendianized_ir(asm_program_t *prog, vector<vine_block_t *> vblocks )
{
  IRDeEndianizer de;

  for (vector<vine_block_t*>::iterator i = vblocks.begin();
       i < vblocks.end(); i++) {
    de.visitvectorStmt((*i)->vine_ir);
  }
...
int main(int argc, char *argv[])
{
  debug_on("warning");
    ...
    //
      ...
      print_deendianized_ir(prog, vine_blocks);

    else if (print_option == 'x' )
      print_xml(prog, vine_blocks);
    //
